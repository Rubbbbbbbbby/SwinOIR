...
class IDSTB8(nn.Module):
    ...
    def forward(self, x):
        block1 = self.gelu(self.block1(x))
        block2 = self.gelu(self.block2(block1))
        c2_dense = self.gelu(torch.cat([block1, block2],1))
        block3 = self.gelu(self.block3(c2_dense))
        c3_dense = self.gelu(torch.cat([block1, block2, block3], 1))
        block4 = self.gelu(self.block4(c3_dense))
        c4_dense = self.gelu(torch.cat([block1, block2, block3, block4], 1))
        block5 = self.gelu(self.block5(c4_dense))
        c5_dense = self.gelu(torch.cat([block1, block2, block3, block4, block5], 1))
        block6 = self.gelu(self.block6(c5_dense))
        c6_dense = self.gelu(torch.cat([block1, block2, block3, block4, block5, block6], 1))
        block7 = self.gelu(self.block7(c6_dense))
        c7_dense = self.gelu(torch.cat([block1, block2, block3, block4, block5, block6, block7], 1))
        block8 = self.gelu(self.block8(c7_dense))
        c8_dense = self.gelu(torch.cat([block1, block2, block3, block4, block5, block6, block7, block8], 1))

        return c8_dense

class IDSTB10(nn.Module):
    def __init__(self, dim, input_resolution, num_heads, window_size, use_checkpoint=False):
        ...
        self.conv = nn.Conv2d(in_channels=18, out_channels=18, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x):
        ...
        block8 = self.gelu(self.block8(c7_dense))
        c8_dense = self.gelu(torch.cat([block1, block2, block3, block4, block5, block6, block7, block8], 1))
        block9 = self.gelu(self.block8(c8_dense))
        c9_dense = self.gelu(torch.cat([block1, block2, block3, block4, block5, block6, block7, block8, block9], 1))
        block10 = self.gelu(self.block10(c9_dense))
        c10_dense = self.gelu(torch.cat([block1, block2, block3, block4, block5, block6, block7, block8, block9, block10], 1))

        return c10_dense

class IDSTB12(nn.Module):
    def __init__(self, dim, input_resolution, num_heads, window_size, use_checkpoint=False):
        ...
        self.conv = nn.Conv2d(in_channels=21, out_channels=21, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x):
        ...
        block8 = self.gelu(self.block8(c7_dense))
        c8_dense = self.gelu(torch.cat([block1, block2, block3, block4, block5, block6, block7, block8], 1))
        block9 = self.gelu(self.block9(c8_dense))
        c9_dense = self.gelu(torch.cat([block1, block2, block3, block4, block5, block6, block7, block8, block9], 1))
        block10 = self.gelu(self.block10(c9_dense))
        c10_dense = self.gelu(torch.cat([block1, block2, block3, block4, block5, block6, block7, block8, block9, block10], 1))
        block11 = self.gelu(self.block11(c10_dense))
        c11_dense = self.gelu(torch.cat([block1, block2, block3, block4, block5, block6, block7, block8, block10, block11], 1))
        block12 = self.gelu(self.block12(c11_dense))
        c12_dense = self.gelu(torch.cat([block1, block2, block3, block4, block5, block6, block7, block8, block10, block11, block12], 1))

        return c12_dense

class IDSTB14(nn.Module):
    def __init__(self, dim, input_resolution, num_heads, window_size, use_checkpoint=False):
        ...
        self.conv = nn.Conv2d(in_channels=24, out_channels=24, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x):
        ...
        c13_dense = self.gelu(torch.cat([block1, block2, block3, block4, block5, block6, block7, block8, block10, block11, block12, block13], 1))
        block14 = self.block14(c13_dense)
        c14_dense = self.gelu(torch.cat([block1, block2, block3, block4, block5, block6, block7, block8, block10, block11, block12, block13, block14], 1))
...
class swinOIR(nn.Module):
    ...
    def __init__(self, img_size=64, patch_size=1, in_chans=3,
        ...
        layer1 = IDSTB8(dim=in_chans, input_resolution=(patches_resolution[0],patches_resolution[1]),
                            num_heads=num_heads, window_size=window_size)
        layer2 = IDSTB10(dim=in_chans, input_resolution=(patches_resolution[0],patches_resolution[1]),
                            num_heads=num_heads, window_size=window_size)
        layer3 = IDSTB12(dim=in_chans, input_resolution=(patches_resolution[0],patches_resolution[1]),
                            num_heads=num_heads, window_size=window_size)
        layer4 = IDSTB14(dim=in_chans, input_resolution=(patches_resolution[0],patches_resolution[1]),
                            num_heads=num_heads, window_size=window_size)
        layer5 = IDSTB12(dim=in_chans, input_resolution=(patches_resolution[0],patches_resolution[1]),
                            num_heads=num_heads, window_size=window_size)
        layer6 = IDSTB10(dim=in_chans, input_resolution=(patches_resolution[0],patches_resolution[1]),
                            num_heads=num_heads, window_size=window_size)
        layer7 = IDSTB8(dim=in_chans, input_resolution=(patches_resolution[0],patches_resolution[1]),
                            num_heads=num_heads, window_size=window_size)
